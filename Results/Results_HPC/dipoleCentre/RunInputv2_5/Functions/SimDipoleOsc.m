function [VRMAT,VRMATosc,VRMATnoise]=SimDipoleOsc(Tend,CSource,CSink,AmpI,USwave,USperiod,idxOscDip,varargin)
...
%     end
    ...
    switch lower(Show_sphere)        
        case 'nopoi'
            varargin2 = horzcat(varargin,{'DOI',idxOscDip,'scale',scale_flag});
            if exist('idxPOI','var')
               varargin2{idxPOI} = [];
            end            
...
switch ParallelCompute_flag
        ...
        switch Include_frequency_flag
            case 0
                ...
                for idp = 1:size(CSource,1) % loop over dipoles
                    ...
                    if display_flag
                        pdpsstep0 = 10^floor(log10(size(CSource,1)-1));
                        if idp == 1
                            pdpsstep = pdpsstep0;
                            fprintf('\t Dipole %i \n',idp)
                        elseif idp >= pdpsstep
    ...
    case 1
        %% NOTE: NEEDS TO BE UPDATED WITH NEW MODELS
        [VRMAT,VRMATosc,VRMATnoise]=SimDipoleOscPC(Tend,CSource,CSink,AmpI,USwave,USperiod,idxOscDip,varargin);
    otherwise
