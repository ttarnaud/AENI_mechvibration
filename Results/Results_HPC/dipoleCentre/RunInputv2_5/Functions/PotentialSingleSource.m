...
if mod(length(varargin)+1,2)
    if ~isempty(varargin)
        ...
        idxtAir = find(strcmpi(varargin,'tAir'))+1;
        if ~isempty(idxtAir)
        tAir = varargin{idxtAir};
        end
        idxscaleflag = find(strcmpi(varargin,'Scale'))+1;
        if ~isempty(idxscaleflag)
        scale_flag = varargin{idxscaleflag};
        end
    end
else
...
if strcmpi(SolutionType,'3sphere')
    xPOI = xPOI.*(RSphere + tSkull + tScalp)./RSphere;
    yPOI = yPOI.*(RSphere + tSkull + tScalp)./RSphere;
    zPOI = zPOI.*(RSphere + tSkull + tScalp)./RSphere;
end

if strcmpi(SolutionType,'4sphere') && scale_flag
    xPOI = xPOI.*(RSphere + tSkull + tScalp + tAir)./RSphere;
    yPOI = yPOI.*(RSphere + tSkull + tScalp + tAir)./RSphere;
    zPOI = zPOI.*(RSphere + tSkull + tScalp + tAir)./RSphere;
end
...
switch lower(SolutionType)
    ...
    case '4sphere'        
        ...
        Gi = @(R,T,iSum,Pi) GetGi(vecnorm(POI,2,2),RBrain,RSkull,RScalp,RAir,sigmaBrain,sigmaSkull,sigmaScalp,sigmaAir,iSum,b0)...
            .*(norm(PosCD,2)/b0).^(iSum-1).*(iSum.*R.*Pi(1,:)'+(-1).*T.*Pi(2,:)');
        % start iteration
        nPosCDMAT = repmat(nPosCD,size(POI,1),1);
        Pi = legendre(idx,CosT);
