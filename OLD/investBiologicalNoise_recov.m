function [RMS,SNR,TSTOP,Out] = investBiologicalNoise_recov(varargin)
...
if ~S4l_flag
    ...
    elseif isempty(OSCindices)
        ...
        if any(strcmpi(eval_method,{'database','validate'}))
            ...
            if length(OSCindices)>max_USwave_varlengths
                ...
                if any(strcmpi(eval_method,{'database','validate'}))
                    Dirus_fun = @(idx) Dirus(find(idx==OSCindices),:);
    ...
    if any(strcmpi(eval_method,{'database','validate'}))
        Dirus_fun = @(idx) [maxvx(idx),maxvy(idx),maxvz(idx)];
...
for irun = 1:irun_end
    ...
    if any(strcmpi(eval_method,{'database','validate'}))
        Dirus_fun_run = @(idx) [maxvx(idx+dpsrun_start),maxvy(idx+dpsrun_start),maxvz(idx+dpsrun_start)];
    ...
    switch lower(eval_method)
        ...
        case 'database'
            ...
            [VRrun,VRDOI_run,VRosc_run,VRnoise_run] = SimDipoleOsc_DB_recov(Tend,CSource(idxdps,:),CSink(idxdps,:),Iarray,USwave,1/fus,idxOscDip,Settings_DB);
        case 'validate'
            [VRrun,VRDOI_run,VRosc_run,VRnoise_run] = SimDipoleOsc(Tend,CSource(idxdps,:),CSink(idxdps,:),Iarray,USwave,1/fus,idxOscDip,Settings);
            if strcmpi(vib_interp_type,'linear')
...
[reconsSignalVR,powerSignalVR,timeReconS] = calcFourierGetsignal(VR,Tsim,POIsidx,fus,fbandwidth,inputSignal,mirror,theta_global,1,'POIstoPLOT',[1:4]);
if ~HPC_flag && PLOT
    mtit('based on all dipoles')
    pause(0.1)
end
%reconstruct based on complete signal plus thermal   
[reconsSignalVRptherm,powerSignalVRptherm] = calcFourierGetsignal(VRptherm,Tsim,POIsidx,fus,fbandwidth,inputSignal,mirror,theta_global,PLOT,'POIstoPLOT',[1:4]);
if ~HPC_flag && PLOT
