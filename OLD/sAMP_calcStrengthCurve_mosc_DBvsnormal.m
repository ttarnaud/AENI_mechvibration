function [RMSall,SNRall,Outall,TSTOPall,Totaldps] = sAMP_calcStrengthCurve_mosc_DBvsnormal(Input)
...
for iAmp = 1:length(Amp)
    for idps = 1:length(Totaldps)
        ...
        % database
        rng(s)
        [RMS,SNR,TSTOP,Out] = investBiologicalNoise(horzcat(Input_iBN,{'eval_method','database'}));
...
RMS_DB_info = {};
liPOIs = size(POIs,1);
Itgen_mat = zeros(length(Amp),length(Totaldps),ceil(max(Totaldps./dps_run)));
Itgen_DB_mat = zeros(length(Amp),length(Totaldps),ceil(max(Totaldps./dps_run)));
VRgen_mat = zeros(length(Amp),length(Totaldps),ceil(max(Totaldps./dps_run)));
VRgen_DB_mat = zeros(length(Amp),length(Totaldps),ceil(max(Totaldps./dps_run)));
if Decomp_struct_flag
    for iAmp = 1:length(Amp)
        for idps = 1:length(Totaldps)
            ...
            Itgen_DB_mat(iAmp,idps,1:length(TSTOPall(iAmp,idps).TSTOP_DB.Itgen)) = TSTOPall(iAmp,idps).TSTOP_DB.Itgen;
            VRgen_DB_mat(iAmp,idps,1:length(TSTOPall(iAmp,idps).TSTOP_DB.VRgen)) = TSTOPall(iAmp,idps).TSTOP_DB.VRgen;
