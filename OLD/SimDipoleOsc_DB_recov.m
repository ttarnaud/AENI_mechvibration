function [VRMAT,VRMATDOI,VRMATosc,VRMATnoise] = SimDipoleOsc_DB(Tend,CSource,CSink,Iarray,USwave,USperiod,idxOscDip,varargin)
...
if strcmpi(vib_interp_type,'linear')
    ...
    Aus_s = @(idx) norm(Dirus(idx_s(idx)))/Aus_DB/(wus); % Database recorded at vibration amplitude = |v|/wus =  1e-6
