function [VRMAT,VRMATDOI,VRMATDOIstat,VRMATOSC,VRMATOSCstat,VRMATstatnoise] = SimDipoleOsc_DB_f(Tend,CSource,CSink,Iarray,USwave,USperiod,idxOscDip,varargin)
%Simulate the potential at certain point of interest based on data from
%database. SimDipoleOSC_DB_f is array based, and uses bsxfun instead of for
        ...
        if any(idx_dpOI(idp)==idxOscDip)
            switch lower(vib_interp_type)
                case 'linear'
                    ...
                    %below similar to for loop in not _f
                    if ~any(dpOI)
                    ...
                    ndpOI_POI = (bsxfun(@cross,POIs',testOI'))';
                    % if POI and dp on same vector
                    if any(~any(ndpOI_POI,2))
                        %ndpOI_POI = cross(POIs(iPOI,:),CSource(idx_dpOI(idp),:));
                        %if ~any(ndpOI_POI)
                        ndpOI_POI(~any(ndpOI_POI,2),:) = repmat(([0,0,1;0,1,0;-1,0,0]*testOI')',sum(~any(ndpOI_POI,2)),1);
                        % if ~any(ndpOI_POI)
                    ...
                    ndpOI_POI = ndpOI_POI./vecnorm(ndpOI_POI,2,2);
                    % determine rotation matrix
                    %rotate first so plane determ by dp and POI is YZ
                    A_plane = calc_RM(ndpOI_POI,ndpDB_POIDB);
                    if any(isnan(A_plane(:)))
                        idxnan = squeeze(any(any(reshape(isnan(A_plane(:)),size(A_plane)),1),2));
                        A_plane(:,:,idxnan) = repmat(eye(3),1,1,sum(idxnan));
                        if any((any(ndpOI_POI-ndpDB_POIDB,2) & idxnan))
                            idxmirror = (any(ndpOI_POI-ndpDB_POIDB,2) & idxnan);
                            A_plane(:,:,idxmirror) = A_plane(:,:,idxmirror) .*(-2*double((ndpOI_POI(idxmirror,:)-ndpDB_POIDB)~=0)+1);
                        end
                        
                    end
                    dpOI_intm = squeeze(sum(A_plane.*testOI,2))';
                    dpOI_intm = dpOI_intm./vecnorm(dpOI_intm,2,2);
                    % rotate dp is on z-axis (rotate around x)
                    A_x = calc_RM(dpOI_intm,[0,0,1]);
                    if any(isnan(A_x(:)))
                        idxnan = squeeze(any(any(reshape(isnan(A_x(:)),size(A_x)),1),2));
                        A_x(:,:,idxnan) = repmat(eye(3),1,1,sum(idxnan));
                        if any((any(dpOI_intm-[0,0,1],2) & idxnan))
                            idxmirror = (any(dpOI_intm-[0,0,1],2) & idxnan);
                            A_x(:,:,idxmirror) = A_x(:,:,idxmirror).*(-2*double((dpOI_intm(idxmirror,:)-[0,0,1])~=0)+1);
                        end
                    end
                    
                    A_plane_cell = mat2cell(A_plane,size(A_plane,1),size(A_plane,2),ones(size(A_plane,3),1));
                    A_x_cell = mat2cell(A_x,size(A_x,1),size(A_x,2),ones(size(A_x,3),1));
                    A = cell2mat(cellfun(@(x,y) x*y,A_x_cell,A_plane_cell,'UniformOutput',false));
                    if any(any(abs(squeeze(sum(A.*testOI,2))'-[0,0,1])>1e-6))
                    ...
                    DirusOI_rot = squeeze(sum(A.*DirusOI,2))';
