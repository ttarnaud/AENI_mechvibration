...
%function that draws little cylinder at POIS position as indicatin of
%electrodes
...
if mod(length(varargin)+1,2)
    if ~isempty(varargin)
        ...
        if any(strcmpi(varargin,'length'))
            L = varargin{find(strcmpi(varargin,'length'))+1};
        end
        if any(strcmpi(varargin,'radius'))
            R = varargin{find(strcmpi(varargin,'radius'))+1};
        end
    end
else
